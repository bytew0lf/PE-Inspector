...
| Relocations | `partial` | Summaries + anomaly totals, machine-aware COFF relocation mapping with spec-matrix tests for ARM/IA64/PPC/SH/SH3E/M32R plus `R3000BE` (MIPS-family behavior), symbol-table-index resolution by true COFF table index, PAIR displacement handling (MIPS/M32R/SH), and undefined-value fallback handling; remaining machine-specific matrices can be expanded further. |
