...
| Sections | `full` | Header decoding (sizes/flags/align), entropy, permissions, padding, overlaps/align checks, section-RVA order and virtual-overlap conformance checks, PE-image no-raw-data pointer consistency checks (`SizeOfRawData == 0` implies `PointerToRawData == 0`), PE-image uninitialized-only raw-size/pointer zero checks, and directory containment summaries. |
