...
- DOS header/stub analysis (relocation table summary + conformance checks for DOS magic (`MZ`) and reserved fields `IMAGE_DOS_HEADER.e_res`/`e_res2`)
...
| --- | --- | --- |
| DOS header + stub | `full` | Header + relocation table summary, plus conformance warnings for non-`MZ` DOS signatures and non-zero reserved words in `IMAGE_DOS_HEADER.e_res` and `IMAGE_DOS_HEADER.e_res2`. |
...
| Sections | `full` | Header decoding (sizes/flags/align), entropy, permissions, padding, overlaps/align checks, section-RVA order, section raw-data order-by-RVA, and virtual-overlap conformance checks, PE-image no-raw-data pointer consistency checks (`SizeOfRawData == 0` implies `PointerToRawData == 0`), PE-image uninitialized-only raw-size/pointer zero checks, and directory containment summaries. |
...
- v58: PE-image section-header conformance now warns when uninitialized-only sections use non-zero `SizeOfRawData` or `PointerToRawData`.
- v59: DOS-header conformance now warns when `IMAGE_DOS_HEADER.e_magic` is non-`MZ` (compatibility parse for `NE`/`LE` retained with strict-mode escalation).
- v60: COFF object optional-header handling now treats non-zero `SizeOfOptionalHeader` as spec-compatible (no false-positive SPEC violation), while continuing to parse object files with optional headers.
- v61: PE-image section conformance now warns when section raw-data layout order does not follow ascending section RVA order.
