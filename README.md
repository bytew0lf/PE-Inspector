...
| Optional header (PE32/PE32+) | `full` | Standard fields + checksum/timestamp decoding + reserved-field conformance checks + full documented subsystem mapping/classification (including `OS2_CUI` and `NATIVE_WINDOWS`) + bounded variable-size optional-header decoding (including reduced headers without directory arrays) + explicit mandatory-field truncation and `NumberOfRvaAndSizes` bounds checks against `SizeOfOptionalHeader`. |
