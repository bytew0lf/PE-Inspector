...
| COFF file header | `full` | Machine/characteristics with full documented machine-name matrix coverage (including canonical `ALPHA`, `ALPHA64 (AXP64)`, `CEE`, and additional IDs such as `R3000BE`/`TARGET_HOST`/`CHPE_X86`), bigobj header support, and image-vs-object COFF deprecation checks including PE-image symbol/line/relocation-pointer rules plus `IMAGE_FILE_RELOCS_STRIPPED`/`IMAGE_FILE_LINE_NUMS_STRIPPED`/`IMAGE_FILE_LOCAL_SYMS_STRIPPED`/`IMAGE_FILE_DEBUG_STRIPPED` consistency validation. |
