...
| Relocations | `full` | Summaries + anomaly totals; machine-aware COFF relocation mapping with full matrix tests across i386/AMD64/ARM/ARM64/IA64/PPC/MIPS/SH/M32R (including `SH3E`/`R3000BE` family behavior) using current documented IA64/PPC constants; true symbol-table-index resolution; PAIR displacement handling plus immediate-predecessor ordering validation (ARM/PPC/MIPS/M32R/SH); IA64 ADDEND immediate-predecessor and payload (non-symbol-index) conformance checks; COFF overflow-relocation (`IMAGE_SCN_LNK_NRELOC_OVFL`) marker parsing/validation; base-relocation HIGHADJ two-slot semantics; 4K-page + 32-bit block-boundary conformance checks; and undefined-value fallback only for non-documented constants. |
